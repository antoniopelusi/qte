...
void editorUpdateRow(erow *row)
{
    ...
    {
        printf("Some line of the edited file is too long for qte\n");
        exit(1);
    }
...
void editorInvDelChar()
{
    ...
    if ((followingRow == NULL) && (filecol == row->size))
        return;
...
#define QTE_QUERY_LEN 256

void editorFind(int fd)
{
    char query[QTE_QUERY_LEN + 1] = {0};
    int qlen = 0;
...
#define QTE_QUIT_TIMES 1
void editorProcessKeypress(int fd)
{
    /* When the file is modified, requires Ctrl-q to be pressed N times
     * before actually quitting. */
    static int quit_times = QTE_QUIT_TIMES;
    ...
    quit_times = QTE_QUIT_TIMES; /* Reset it to the original value. */
}
...
int main(int argc, char **argv)
{
    if (argc != 2)
    {
        fprintf(stderr, "Usage: qte <filename>\n");
        exit(1);
    }
